...
public class DatabaseService : MonoBehaviour
{
    ...
    private const string DatabaseName = "portal.db";
 ...
 Application.dataPath + "/StreamingAssets/" + DatabaseName;  // this is the path to your StreamingAssets in iOS
    ...
    {
        return _database.Table<Map>();
    }

    public static void SaveMap(string code, string mapJson)
    {
        _database.Insert(new Map {Code = code, MapJson = mapJson});
    }

    public static void DeleteMap(int mapId)
    {
        _database.Delete(new Map {Id = mapId});
    }
